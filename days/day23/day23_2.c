...
int main() {
    ...
    do {
        ...
        for (int j = 0; j < elves->size; j++) {
            ...
            PointerList *current_proposal_row = get_pointer(proposals_by_row, (py + proposals_by_row->size * (round / proposals_by_row->size + 1)) % proposals_by_row->size);
            ...
            for (int k = 0; k < current_proposal_row->size; k++) {
                ...
                if (current_proposal->px == px && current_proposal->py == py) {
                    ...
                    PointerList *new_removed_proposal_row = get_pointer(proposals_by_row, (current_proposal->py + proposals_by_row->size * (round / proposals_by_row->size + 1)) % proposals_by_row->size);
                    add_pointer(new_removed_proposal_row, removed_proposal);
                    break;
                }
            }

            PointerList *new_proposal_row = get_pointer(proposals_by_row, (py + proposals_by_row->size * (round / proposals_by_row->size + 1)) % proposals_by_row->size);
