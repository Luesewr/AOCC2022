...
void addNeighbourCell(PointerList *grid, int cx, int cy, int required_height, int required_score, QueueElement **tail) {
    /*
     * Get the cell in the grid that corresponds with the position given. (the neighbour cell)
     */
    Cell *cell = get_pointer(get_pointer(grid, cy), cx);

    /*
     * Check if the neighbour is valid according to the rules given.
     */
    if (cell != NULL && cell->height <= required_height && cell->shortest > 999999) {

        /*
         * Create a new queue element, update the shortest path length of the neighbour cell
         * and add the neighbour queue element to the end of the queue.
         */
