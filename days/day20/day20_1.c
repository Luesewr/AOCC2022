...
int main() {
    ...
    if (fgets(line, 10, file) != NULL) {
        ...
        start_element->number = (int) strtol(line, NULL, 10);
    ...
    while (fgets(line, 10, file) != NULL) {
        ...
        current->number = (int) strtol(line, NULL, 10);
        current->prev = prev;
        prev->next = current;
    ...
    for (int i = 0; i < length; i++) {
        ...
        int abs_number = abs(current->number) % (length - 1);
